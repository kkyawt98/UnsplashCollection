...
class LikesViewHolder(
    ...
    fun bind(users: UsersLikedItem) {
        Glide.with(v).load(users.urls.full)
            ...
            .error(android.R.drawable.stat_notify_error).into(v.img_home)

        itemView.txt_user_name.text = users.user.name

        Glide.with(v).load(users.user.profileImage.medium)
            .placeholder(android.R.drawable.progress_indeterminate_horizontal)
            .error(android.R.drawable.stat_notify_error).into(v.img_user)
    ...
    interface ClickListener {
        fun Onclick(photo: UsersLikedItem)
    }
}