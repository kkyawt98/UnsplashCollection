...
class LikesAdapter (var likesList: List<UsersLikedItem> = emptyList()) : RecyclerView.Adapter<LikesViewHolder>(){
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LikesViewHolder {
        return LikesViewHolder(parent.context.bindView(R.layout.item_home,parent))
    }

    override fun onBindViewHolder(holder: LikesViewHolder, position: Int) = holder.bind(likesList[position])
